...
# Install Logstash
RUN cd /usr/local; \
  wget -q https://artifacts.elastic.co/downloads/logstash/logstash-5.6.3.tar.gz; \
  mkdir /usr/local/logstash; \
  tar -zxf logstash-5.6.3.tar.gz -C /usr/local/logstash; \
  mkdir /opt/logstash; \
  rm -f logstash-5.6.3.tar.gz
